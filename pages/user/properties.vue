<template>
    <view class="content w-100">
    	...
    	<view class="data m-b-50" >
			<view class="font-size-normal m-l-10 m-t-10 m-b-10">{{header_txt}}</view>
			<view class="m-l-10 m-r-10" v-if="propertyList != undefined && propertyList != null && propertyList.length > 0">
								...
								if(uni.getStorageSync("language") == "en") {
									if(that.detailInfo.unit_featured.length > 0) {
										for(var i = 0; i < that.detailInfo.unit_featured.length; i++) {
											that.detailInfo.unit_featured[i].price_total = that.detailInfo.symbol + (that.detailInfo.unit_featured[i].price_total / 1000000).toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") + "M" 
										}
									}
								}
								else {
